...
def load_data(data_path, others_path, graphene_path):
    size_thre = 100
    ...
    for exp_name in exp_names:
        graphene_feats.append(load_one_class(os.path.join(data_path, exp_name), os.path.join(graphene_path, exp_name), size_thre))

    graphene_feats = np.concatenate(graphene_feats, axis=0)
    ...
    others_feats = []
    for exp_name in exp_names:
        others_feats.append(load_one_class(os.path.join(data_path, exp_name), os.path.join(others_path, exp_name), size_thre))

    others_feats = np.concatenate(others_feats, axis=0)

    num_other = others_feats.shape[0]
    print("number of others: {}".format(num_other))

    return graphene_feats, others_feats
...
def main():
    ...
    classifier_save_path_v1 = '../results/data_sep2019_script/graphene_classifier_binary_fea-%s'%(args.color)
    ...
    classifier_save_path_v2 = '../results/10222019G wtih Suji_script/center_patch_500_500/graphene_classifier_binary_fea-%s'%(args.color)
    if not os.path.exists(classifier_save_path_v2):
        os.makedirs(classifier_save_path_v2)
    ...
    if os.path.exists(feat_save_path_v2):
        ...
        graphene_feats_v2, other_feats_v2 = load_data(data_path, others_path, graphene_path)
        feats = {}
        feats['graphene_feats'] = graphene_feats_v2
        feats['other_feats'] = other_feats_v2
        pickle.dump(feats, open(feat_save_path_v2, 'wb'))
    ...
    classify(graphene_feats, other_feats, classifier_save_path_v2)
