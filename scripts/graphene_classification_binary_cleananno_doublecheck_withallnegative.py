...
def readdb(all_dbname):
    ...
    c = conn.cursor()
    c.execute('SELECT expname_subexpname_imflakeid, thicklabel FROM annotab')
    db = c.fetchall()
    ...
    for i in range(len(db)):
        expname_imflakeid = db[i][0]
        expname, subexpname, imflakeid = expname_imflakeid.split('+')
        # expnames.append(expname)
        # subexpnames.append(subexpname)
        ...
        oriname_flakeids.append([expname, subexpname, flake_oriname+'-'+str(flake_id), flake_oriname, flake_id, label_id])
...
def load_one_image(color_fea, flake_path, fname, data_path, size_thre, annotated_names, annotated_labels):
    ...
    # print(labeled_feats)
    return labeled_feats, unlabeled_feats, labeled_flake_ids, unlabled_flake_ids, labeled_labels
...
def load_one_dir(sub_anno_data, sub_data_path, sub_flake_path):
    ...
    unlabeled_flake_ids = [feat[3] for feat in all_feats]
    labeled_labels = [feat[4] for feat in all_feats]

    labeled_flake_name_ids = []
    unlabeled_flake_name_ids = []
    ...
    for i in range(len(unlabeled_flake_ids)):
        if len(unlabeled_flake_ids[i]) > 0:
            unlabeled_flake_name_ids.extend([flake_names[i] + '-' +str(x) for x in unlabeled_flake_ids[i]])
    ...
    return labeled_feats, unlabeled_feats, labeled_flake_name_ids, unlabeled_flake_name_ids, labeled_labels
...
def main():
    ...
    result_classify_path = '../results/data_111x_individual_script/graphene_classifier_with_clean_anno_doublecheck_allnegative_colorfea-%s'%(args.color_fea)
    annotation_path = '../data/double_check_anno_graphene_youngjae/anno_user-youngjae.db'
    ...
    if os.path.exists(feat_save_path):
        ...
        for exp_name in exp_names:
            ...
            for sname in subexp_names:
                ...
                if len(sub_anno_data) > 0:
                    labeled_feats, unlabeled_feats, labeled_flake_name_ids, unlabeled_flake_name_ids, labels = load_one_dir(sub_anno_data, os.path.join(data_path, exp_name, sname), os.path.join(result_path, exp_name, sname))
                    ...
                    unlabeled_flake_name_ids = [(exp_name, sname, unlabeled_flake_name_ids[i]) for i in range(len(unlabeled_flake_name_ids))]
                    all_labeled_flake_name_ids.extend(labeled_flake_name_ids)
                    all_unlabeled_flake_name_ids.extend(unlabeled_flake_name_ids)
        ...
        feats['unlabeled_flake_name_ids'] = all_unlabeled_flake_name_ids
    ...
    # treat unlabeled features as negative features
    all_feats = np.concatenate([all_labeled_feats, all_unlabeled_feats], 0)
    ...
    for method in methods:
        for C in Cs:
            ...
            if os.path.exists(clf_save_path):
                ...
                if method == 'linearsvm':
                    clf = LinearSVC(random_state=0, tol=1e-5, C=C, max_iter=9e4, class_weight={0:1, 1:num_others/num_graphene})#, multi_class='crammer_singer')
            ...
            train_pred_cls = clf.predict(all_feats)
            train_pred_scores = clf.decision_function(all_feats)
