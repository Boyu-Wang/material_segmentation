...
#     HoverWs.append(f_mask_height / f_mask_width)
    ...
    val_feats -= mean_feat
    val_feats = val_feats / np.linalg.norm(val_feats, axis=1, keepdims=True)
    ...
    C = hyperparams['C']
    # C = 10
    clf_save_path = os.path.join(clf_path, 'classifier-%s-%f.p'%(method, C))
    ...
    clf_vis_path = os.path.join('../results/data_jan2019_script/flakeglue_clf', subexp_name, 'vis', '%s-%f'%(method, C))
