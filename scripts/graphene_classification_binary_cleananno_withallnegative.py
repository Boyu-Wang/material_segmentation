...
Aslo add all other unlabeled flakes as non-graphene flakes.
"""
...
parser.add_argument('--color_fea', default='contrast-bg-shape', type=str, help='which color feature to use: contrast, ori, both, contrast-bg, ori-bg, both-bg, contrast-bg-shape')
...
def main():
    ...
    result_classify_path = '../results/data_111x_individual_script/graphene_classifier_with_clean_anno_allnegative_colorfea-%s'%(args.color_fea)
    ...
    for method in methods:
        for C in Cs:
            ...
            if os.path.exists(clf_save_path):
                ...
                if method == 'linearsvm':
                    clf = LinearSVC(random_state=0, tol=1e-5, C=C, max_iter=9e4, class_weight={0:1, 1:500})#, multi_class='crammer_singer')
