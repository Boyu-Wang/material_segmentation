"""
hierarchical classifier: glue/flake, then thick/thin
Complete annotation
...
hyperparams = { 'clf_method': 'hielinearsvm', # which classifier to use (linear): 'ridge', 'hielinearsvm', 'hierbfsvm'
...
def main():
    ...
    C = hyperparams['C']
    # C = 10
    Cs = [0.001, 0.01, 0.1, 0.25, 0.5, 0.75, 1, 2.5, 5, ]
        ...
        if os.path.exists(clf_save_path):
            ...
            if method == 'hielinearsvm':
                ...
                base_estimator = make_pipeline(
                    ...
                    SVC(gamma='auto', kernel="linear", probability=True, C=C),
                )
