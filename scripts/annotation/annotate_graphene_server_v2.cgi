...
# http://vision.cs.stonybrook.edu/cgi-bin/AnnoEx/cgi-bin/annotate_graphene_server_v2.cgi?fea=contrast-bg-shape&expinfo=PDMS-QPress 60s_2&size=100-5000&id=0&user=test0123
...
# rslt_path = '../rslt/data_111x_individual_script/test_classify_graphene_colorfea-%s_clf-linear_data-doublecheck_topvisualize-100_patch-withori/'%(fea_type)
rslt_path = '../rslt/data_111x_individual_script/test_classify_graphene_colorfea-%s_clf-linear_data-cleananno_topvisualize-100_patch-withori_2.0_100/'%(fea_type)
...
# subannos_db_path = os.path.join('../data/anno_graphene_v2_%s/'%user, exp_name, subexp_name)
subannos_db_path = os.path.join('../data/anno_graphene_v3_%s/'%user, exp_name, subexp_name)
...
if (id >= num_group):
    ...
    print(""" 
        <div id="form_container">      
            <form id="form_390674" class="appnitro"  method="post" action="annotate_graphene_server_v2.cgi?fea=%s&expinfo=%s&size=%s&id=%d&user=%s">      
    ...
    for r_i in range(n_row):
        ...
        for c_i in range(len(row_names)):
            ...
            # print(os.path.join(subrslt_path, row_names[c_i]))
            img_to_read = PIL.Image.open(os.path.join(subrslt_path, row_names[c_i]))
            # img_to_read = scipy.misc.imread(os.path.join(subrslt_path, row_names[c_i]))
            # img_to_read = imageio.imread(os.path.join(subrslt_path, row_names[c_i]))
            # img_to_read = matplotlib.image.imread(os.path.join(subrslt_path, row_names[c_i]))
