...
# online learning for flake thickness annotation
# http://vision.cs.stonybrook.edu/cgi-bin/AnnoEx/cgi-bin/annotate_online_thickness_server.cgi?expinfo=YoungJaeShinSamples_4&sizethre=784&id=0&user=test0123
...
if sizethre == 784:
	rslt_path = '../rslt/data_jan2019_script/cluster_sort_784_thickness_online/'
# elif sizethre == 100:
...
subannos_db_path = os.path.join('../data/anno_thickness_online%d/'%(sizethre), exp_name, subexp_name+ '_' + clf_method, user)
...
if num_tolabel > 0:
	...
	print(""" 
		<div id="form_container">      
			<form id="form_390674" class="appnitro"  method="post" action="annotate_online_thickness_server.cgi?expinfo=%s&sizethre=%d&id=%d&user=%s">      
	...
	for c_i in range(len(top_thin_idxs)):
		img_path = os.path.join(subonline_rslt_path[2:], "img-%s.png"%(tolabel_names[top_thin_idxs[c_i]]))
		print("""
	...
	print("""<tr> """)
	for c_i in range(len(top_thin_idxs)):
		ck_str = "checked" if top_thin_labels[c_i] else ""
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="thin" %s > thin </td>""" %(tolabel_names[top_thin_idxs[c_i]], c_i, ck_str))
	print("""</tr> """)
	# the label part, thick
	print("""<tr> """)
	for c_i in range(len(top_thin_idxs)):
		ck_str = "checked" if not top_thin_labels[c_i] else ""
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="thick" %s > thick </td>""" %(tolabel_names[top_thin_idxs[c_i]], c_i, ck_str))
	print("""</tr> """)
	# the label part, others
	print("""<tr> """)
	for c_i in range(len(top_thin_idxs)):
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="others" > others </td>""" %(tolabel_names[top_thin_idxs[c_i]], c_i))
	print("""</tr> """)
	...
	for c_i in range(len(top_thick_idxs)):
		img_path = os.path.join(subonline_rslt_path[2:], "img-%s.png"%(tolabel_names[top_thick_idxs[c_i]]))
		print("""
	...
	print("""<tr> """)
	for c_i in range(len(top_thick_idxs)):
		ck_str = "checked" if top_thick_labels[c_i] else ""
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="thin" %s > thin </td>""" %(tolabel_names[top_thick_idxs[c_i]], c_i, ck_str))
	print("""</tr> """)
	# the label part, thick
	print("""<tr> """)
	for c_i in range(len(top_thick_idxs)):
		ck_str = "checked" if not top_thick_labels[c_i] else ""
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="thick" %s > thick </td>""" %(tolabel_names[top_thick_idxs[c_i]], c_i, ck_str))
	print("""</tr> """)
	# the label part, others
	print("""<tr> """)
	for c_i in range(len(top_thick_idxs)):
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="others" > others </td>""" %(tolabel_names[top_thick_idxs[c_i]], c_i))
	print("""</tr> """)
