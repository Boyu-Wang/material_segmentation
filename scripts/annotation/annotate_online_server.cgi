...
# online learning for flake annotation
# http://vision.cs.stonybrook.edu/cgi-bin/AnnoEx/cgi-bin/annotate_online_server.cgi?expinfo=YoungJaeShinSamples_4&sizethre=784&id=0&user=test0123
...
if sizethre == 784:
	rslt_path = '../rslt/data_jan2019_script/cluster_sort_784_online/'
# elif sizethre == 100:
...
with open(os.path.join(subonline_rslt_path, 'all_test_names.txt'), 'r') as f:
	all_test_names = f.readlines()
all_test_names = [n.strip() for n in all_test_names]

all_test_feas = pickle.load(open(os.path.join(subonline_rslt_path, 'all_test_feas.p'), 'rb'))

subannos_db_path = os.path.join('../data/anno_online_%d/'%(sizethre), exp_name, subexp_name+ '_' + clf_method, user)
...
if not os.path.exists(os.path.join(subannos_db_path, 'classifier-%s_b-%d.p'%(clf_method, -1))):
	src_path = os.path.join(subonline_rslt_path, 'classifier-%s_b-%d.p'%(clf_method, -1))
	des_path = os.path.join(subannos_db_path, 'classifier-%s_b-%d.p'%(clf_method, -1))
...
if len(last_labeled_test_names) > 0:
	...
	c = conn.cursor()
	last_clf = pickle.load(open(os.path.join(subannos_db_path, 'classifier-%s_b-%d.p'%(clf_method, id-2)), 'rb'))
	...
	pickle.dump(to_save, open(os.path.join(subannos_db_path, 'classifier-%s_b-%d.p'%(clf_method, id-1)), 'wb'))
 ...
 # 					<a href="annotate_online_server.cgi?expinfo=%s&sizethre=%d&id=%d&user=%s">Click here to relabel them </a> </h2>""" % (expinfo, sizethre, id - 1, user))
	...
	last_clf = pickle.load(open(os.path.join(subannos_db_path, 'classifier-%s_b-%d.p'%(clf_method, id-1)), 'rb'))
	Cn_inv = last_clf['Cn_inv'] 
	last_coef = last_clf['coef'] # [dim, 1]
...
if num_tolabel > 0:
	...
	# run the classifier, find the most confident glue(>0) or flake(<0) candidates
	...
	# find top glue(positive) and top flake(negative)
	tolabel_sort_idxs = np.argsort(tolabel_scores) # from small to large

	top_glue_idxs = tolabel_sort_idxs[-min(num_tolabel/2, n_column):]
	top_flake_idxs = tolabel_sort_idxs[:min(num_tolabel - num_tolabel/2, n_column)]

	top_glue_labels = [tolabel_scores[x]>0 for x in top_glue_idxs]
	top_flake_labels = [tolabel_scores[x]>0 for x in top_flake_idxs]

	# print(tolabel_scores)
	# print(tolabel_sort_idxs)
	# print(num_tolabel)
	# print(top_glue_idxs)
	batch_names = [tolabel_names[kk] for kk in top_glue_idxs ] + [tolabel_names[kk] for kk in top_flake_idxs]
	...
	print(""" 
		<div id="form_container">      
			<form id="form_390674" class="appnitro"  method="post" action="annotate_online_server.cgi?expinfo=%s&sizethre=%d&id=%d&user=%s">      
	...
	# the image part
	print("""<tr> """)
	for c_i in range(len(top_glue_idxs)):
		img_path = os.path.join(subonline_rslt_path[2:], "img-%s.png"%(tolabel_names[top_glue_idxs[c_i]]))
		print("""
	...
	print("""<tr> """)
	for c_i in range(len(top_glue_idxs)):
		ck_str = "checked" if top_glue_labels[c_i] else ""
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="glue" %s > glue </td>""" %(tolabel_names[top_glue_idxs[c_i]], c_i, ck_str))
	print("""</tr> """)
	# the label part, flake
	print("""<tr> """)
	for c_i in range(len(top_glue_idxs)):
		ck_str = "checked" if not top_glue_labels[c_i] else ""
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="flake" %s > flake </td>""" %(tolabel_names[top_glue_idxs[c_i]], c_i, ck_str))
	print("""</tr> """)
	# the label part, others
	print("""<tr> """)
	for c_i in range(len(top_glue_idxs)):
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="others" > others </td>""" %(tolabel_names[top_glue_idxs[c_i]], c_i))
	print("""</tr> """)

	### flake
	# the image part
	print("""<tr> """)
	for c_i in range(len(top_flake_idxs)):
		img_path = os.path.join(subonline_rslt_path[2:], "img-%s.png"%(tolabel_names[top_flake_idxs[c_i]]))
		print("""
	...
	print("""<tr> """)
	for c_i in range(len(top_flake_idxs)):
		ck_str = "checked" if top_flake_labels[c_i] else ""
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="glue" %s > glue </td>""" %(tolabel_names[top_flake_idxs[c_i]], c_i, ck_str))
	print("""</tr> """)
	# the label part, flake
	print("""<tr> """)
	for c_i in range(len(top_flake_idxs)):
		ck_str = "checked" if not top_flake_labels[c_i] else ""
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="flake" %s > flake </td>""" %(tolabel_names[top_flake_idxs[c_i]], c_i, ck_str))
	print("""</tr> """)
	# the label part, others
	print("""<tr> """)
	for c_i in range(len(top_flake_idxs)):
		print("""
	    <td align="center"><input type="radio" name=%s data-col=%d value="others" > others </td>""" %(tolabel_names[top_flake_idxs[c_i]], c_i))
	print("""</tr> """)
