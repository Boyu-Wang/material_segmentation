...
def classify_one_image(img_name, info_name, classifier, norm_fea, new_img_save_path):
    ...
    if not os.path.exists(info_name):
        ...
        return total_num_flake, total_num_thick, total_num_thin, total_num_glue
...
def classify_one_subexp(subexp_dir, rslt_dir, result_classify_save_path, norm_fea, classifier):
    ...
    total_counts = Parallel(n_jobs=args.n_jobs)(delayed(classify_one_image)(os.path.join(subexp_dir, img_names[i]), os.path.join(rslt_dir, img_names[i][:-4] + '.p'),
                       classifier, norm_fea, os.path.join(result_classify_save_path, img_names[i])) for i in range(len(img_names)))
    total_num_flake = sum([total_counts[i][0] for i in range(len(img_names))])
    total_num_thick = sum([total_counts[i][1] for i in range(len(img_names))])
    total_num_thin = sum([total_counts[i][2] for i in range(len(img_names))])
    total_num_glue = sum([total_counts[i][3] for i in range(len(img_names))])

    output_text = 'name, total, thick, thin, glue\n'
    output_text += '%s, %d, %d, %d, %d\n'%(subexp_dir, total_num_flake, total_num_thick, total_num_thin, total_num_glue)
    output_text += '%s, %.2f, %.2f, %.2f, %.2f\n'%(subexp_dir, total_num_flake/total_num_flake, total_num_thick/total_num_flake, total_num_thin/total_num_flake, total_num_glue/total_num_flake)
    with open(os.path.join(result_classify_save_path, 'thick_thin_glue.txt'), 'w') as f:
        f.write(output_text)
...
def main():
    ...
    result_classify_path = '../results/data_111x_individual_script/test_classify_thickthinglue_clf-linearsvm_data-jan'
