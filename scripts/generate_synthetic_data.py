...
def generate_hv(thickness, max_thickness=50):
    """
...
def generate_flake(bg_hsv, im_size):
    flake_width = np.random.randint(15, 30)
    flake_height = np.random.randint(15, 30)
    ...
    thickness = np.random.randint(1, 50)
...
def generate_one_img(bg_hsv, im_size, save_dir, img_idx):
    ...
    im_hsv = bg_hsv[im_h:im_h+im_size[0], im_w:im_w+im_size[1], :]
